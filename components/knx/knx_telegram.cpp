...
std::string KnxTelegram::get_target_group(){
  std::string target =
        std::to_string(this->get_target_main_group())   + "/" +
        std::to_string(this->get_target_middle_group()) + "/" +
        std::to_string(this->get_target_sub_group());
  return target;
}
...
uint8_t KnxTelegram::get_4bit_steps_value() {
  ...
  return static_cast<uint8_t>(get_first_data_byte() & 0b00000111);
}
...
void KnxTelegram::set_2byte_float_value(float value) {
  ...
  long m = std::lround(v) & 0x7FF;
  short msb = (short) (exponent << 3 | m >> 8);
  if (value < 0.0f) msb |= 0x80;
  buffer[8] = msb;
  buffer[9] = static_cast<uint8_t>(m);
}
...
float KnxTelegram::get_2byte_float_value() {
  ...
  if (buffer[8] & 0b10000000) {
    return ((-2048 + mantissa) * 0.01) * std::pow(2.0, exponent);
  }

  return (mantissa * 0.01) * std::pow(2.0, exponent);
}
...
void KnxTelegram::set_14byte_value(std::string value) {
  // Define
  char _load[15];
  ...
  // copy up to 14 chars and ensure null termination
  size_t len = value.size() < 14 ? value.size() : 14;
  if (len > 0) {
    memcpy(_load, value.data(), len);
  }
  _load[len] = '\0';
  buffer[8 + 0] = _load [0];
  buffer[8 + 1] = _load [1];
